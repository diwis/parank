...
# 2) a value
# 3) p value (factor in exponential formula)
# 4) current year (setting current time)
# 5) convergence error
...
if(len(sys.argv) < 7):
	print "Usage: ./parank.py <input_file> <alpha> <beta> <gamma> <current_year> <convergence_error>"
	sys.exit(0)
else:
...
# Output in new file
os.popen("export LC_ALL=C; cat paper_graphstep.txt | while IFS=$'\t' read paper dats score year attachment yearweight; do final_score=${dats##*|}; echo  \"${paper}\t${dats}\t${score}\t${year}\t${attachment}\t${yearweight}\t${final_score}\"; done | sort -k7 -gr > " + parank_input + "_a" + str(alpha) + "_b" + str(beta) + "_c" + str(gamma) + "_year" + str(current_year) + "_i" + str(iterations) + ".txt")
